import {greenify, resetAllBarColors} from "./CommonMethods/commonMethods";
...
export function animate(idx, res, arrayBars, completedAnimations, toBeSortedIndex, ANIMATION_SPEED_MS, comparisons) {
    const stage = idx % 3;

    if (stage === 0) {
        const [barOneIdx, barTwoIdx] = res[idx];
        ...
        const [indexJ, indexJ1] = res[idx - 1];
        const [indexJVal, indexJ1Val] = res[idx];
        ...
        const [indexJ, indexJ1] = res[idx - 2];
        const [indexJVal, indexJ1Val] = res[idx - 1];
        ...
        setTimeout(() => {
            ...
            greenify(completedAnimations, res, arrayBars);
        }, (idx + 1) * ANIMATION_SPEED_MS);
    }
    return [completedAnimations, toBeSortedIndex, comparisons];
}